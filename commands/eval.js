...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('eval')
        .setDescription('Schedule a eval')
        .addStringOption(option => 
            option.setName('datetime')
                .setDescription('Day and time of the evaluation (MST)')
                .setRequired(true)),
    async execute(interaction) {
        ...
        const tryoutEmbed = new EmbedBuilder()
            ...
            .setDescription(`A evaluation has been scheduled by ${user.tag}.`)
            .addFields(
                { name: 'Date and Time', value: dateTime },
                { name: 'Dress Code:', value: 'The dress code is that all items must be removed on your avatar. Including faces.nod' },
        ...
        try {
            await channel.send('<@&1225146583474241739> <@&1264258792762118156>');
